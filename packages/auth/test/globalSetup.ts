...
const TEST_MONGO_ROOT_USER = process.env.TEST_MONGO_ROOT_USER
const TEST_MONGO_ROOT_PASSWORD = process.env.TEST_MONGO_ROOT_PASSWORD
...
export async function setup(project: TestProject): Promise<void> {
  ...
  const credentials =
    TEST_MONGO_ROOT_USER && TEST_MONGO_ROOT_PASSWORD
      ? `${encodeURIComponent(TEST_MONGO_ROOT_USER)}:${encodeURIComponent(TEST_MONGO_ROOT_PASSWORD)}@`
      : ''
  const TEST_ROOT_MONGODB_URI = `mongodb://${credentials}${host}:${port}/admin?directConnection=true&replicaSet=rs0`
